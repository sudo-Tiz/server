...
#elif defined HAVE_SSE42 && defined __GNUC__
# include <cpuid.h>
...
USE_SSE42
static unsigned crc32c_sse42(unsigned crc, const void* buf, size_t size)
{
  const uint8_t *p = static_cast<const uint8_t *>(buf);
  const uint8_t *e = p + size;
  uint64_t l = crc ^ 0xffffffffu;
...
#elif defined HAVE_SSE42
# ifdef USE_VPCLMULQDQ
  if (ChosenExtend == crc32c_vpclmulqdq)
    return "Using AVX512 instructions";
# endif
# if SIZEOF_SIZE_T == 8 && defined HAVE_PCLMUL
